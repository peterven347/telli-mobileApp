...
import { getIssue, url } from "../../../../utils/https";
...
let delay = 125
let totalTimeout = 0
const RenderSectionHeader = memo(({ title, sectorId, skip, setTitle, setSectorId, setIssueModalVisible, setEditSectorModalVisible }) => {
    const { currDomainId, fetchedSectorIds } = useDomain.getState()
    useEffect(() => {
        let timer;
        const timeout = totalTimeout
        // console.log(fetchedSectorIds)
        if (!fetchedSectorIds.includes(sectorId)) {
            timer = setTimeout(() => {
                getIssue(currDomainId, sectorId, skip)
            }, timeout);
            delay += 125;
            totalTimeout += delay
        }
    ...
    return (
        <View style={styles.sectionHeader}>
            <Pressable onPress={() => { setTitle(title); setSectorId(sectorId); setEditSectorModalVisible(true) }}>
                <Text style={{ fontWeight: "600", backgroundColor: "#eee8", paddingHorizontal: 4, borderRadius: 2 }}>{title}</Text>
            </Pressable>
            <Pressable onPress={() => { setTitle(title); setSectorId(sectorId); setIssueModalVisible(true) }} style={{ paddingStart: "10%" }}>
        ...
        <>
            ...
            <Pressable onLongPress={() => { }} style={{ paddingEnd: 1 }}>
                <View style={styles.renderNote}>
                    ...
                    <Text
                        style={styles.note}
                        numberOfLines={noOfLines[item._id] ? undefined : item.pictures?.length === 0 ? 10 : 6}
                        onPress={() => {
                            setNoOfLines(prev => ({
                                ...prev,
                ...
                <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
                    {pictures?.map((i, index) => (
                        <MemoizedImage key={i} index={index} len={item.pictures?.length} uri={i} fn={() => {
                            // setModalImgList(item.pictures?.map(i => item._id ? `${url}/img-file/issuesImg/${i}` : i));

                            // setModalImgList(prev => prev.map(i => prev[index]))
                            // flatListRef.current?.scrollToIndex({ index: index })
                            // setModalImgList(prev => prev.map((i, index) => `${url}/img-file/issuesImg/${item.pictures[index]}`))
                            // setTimeout(() => {
                            // }, 2000);
                            openImage(index)
                            // setTimeout(() => {
                            //     setModalImgList(prev => prev.map(i => {
                            //         if (prev.indexOf(i) === 1) {
                            //             return prev[index]
                            //         }
                            //         return i
                            ...
                            // setTimeout(() => {
                            //     setModalImgList(prev => [prev[2], ...prev])
                            // }, 2000);
                        }} />
                        // <Text>{`${url}/img-file/issuesImg/${i}`}</Text>
                    ))}
                </View>
                <View style={styles.shareView}>
                    {
                        item._id ? <Text style={{ fontSize: 10, minWidth: "20%", color: "#000a" }}>{dateFormatter(item?._id)}</Text>
                            : <Mci name="timer-sand" size={12} color="#999" />
                    }
                    <View style={{ flexDirection: "row", alignItems: "center" }}>
                        {item._id &&
                            <>
                                <Mci name={user?.issues_voted?.includes(item._id) ? "checkbox-marked" : "checkbox-blank-outline"} size={18} color={user?.issues_voted?.includes(item._id) ? "#6eafb2" : "#888"} marginEnd={18} onPress={() => vote(item?._id)} />
...
export default function SideComponent({ navigation, dropVisible, setDropVisible }) {
    ...
    //     // Update the set of visible item IDs
    ...
    return (
        <>
            ...
            <SectionList
                sections={domains?.find(i => i._id === currDomainId)?.sectors?.filter(i => clickedTitle ? i.title === clickedTitle : true)?.sort((a, b) => (b.time || 0) - (a.time || 0)) ?? []}
                keyExtractor={(item) => item._id || item.id}
                renderItem={({ item, section }) => <RenderItem item={item} />}
                renderSectionHeader={({ section: { _id, title, data } }) =>
                    (<RenderSectionHeader title={title} sectorId={_id} skip={data.length} setTitle={setTitle} setSectorId={setSectorId} setIssueModalVisible={setIssueModalVisible} setEditSectorModalVisible={setEditSectorModalVisible} />)}
